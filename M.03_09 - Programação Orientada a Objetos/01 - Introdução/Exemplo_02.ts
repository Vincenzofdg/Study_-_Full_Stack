class Email {
  ...
  get content(): string { return `From ${this._from} to ${this._to} ${this.subject} ${this._message}`; }
}

class MailList {
  private mailList;

  constructor(list: Email[]) {
    this.mailList = list;
  }
...
const personalMailList = new MailList([email01, email02, email03]);
const professionalMailList = new MailList([email04, email05]);
